...
def create_clip_with_title(input_video, start_time, end_time, title, output_filename):
    """Create a video clip with title overlay"""
    ...
    # FFmpeg command to extract clip and add title overlay
    cmd = f"""ffmpeg -i "{input_video}" -ss {start_time} -t {duration} \
-vf "drawtext=text='{escaped_title}':fontfile=/System/Library/Fonts/PingFang.ttc:fontsize=24:fontcolor=white:x=(w-text_w)/2:y=30:box=1:boxcolor=black@0.7:boxborderw=5" \
...
def main():
    ...
    for i, clip in enumerate(clips_data, 1):
        ...
        success = create_clip_with_title(
            input_video,
            clip["start"],
            clip["end"], 
            clip["title"],
